...
public class StocksTracingMiddleware(RequestDelegate next)
{
	public async Task InvokeAsync(HttpContext context)
	{
		...
		using var activity = StocksActivitySource.Instance.StartActivity($"{StocksActivitySource.Instance.Name}.{operationName}");
	...
	private static string GetOperationName(HttpContext context)
	{
		...
		return $"{StocksActivitySource.Instance.Name}.{methodMapping}";
	}
}
