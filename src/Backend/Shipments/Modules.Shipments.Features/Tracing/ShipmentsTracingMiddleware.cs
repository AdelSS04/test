...
public class ShipmentsTracingMiddleware(RequestDelegate next)
{
	private static readonly ActivitySource ActivitySource = new(ShipmentsTracingConsts.ActivityModuleName);
	...
	public async Task InvokeAsync(HttpContext context)
	{
		...
		using var activity = ActivitySource.StartActivity($"{ShipmentsTracingConsts.ModuleName}.{operationName}");

		activity?.SetTag("module", ShipmentsTracingConsts.ModuleName);
	...
	private static string GetOperationName(HttpContext context)
	{
		...
		return $"{ShipmentsTracingConsts.ModuleName}.{methodMapping}";
	}
}
